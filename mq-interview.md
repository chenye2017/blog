...
* Go  的并发模型和 php的并发模型的区别

  ```
  我理解感觉更像是php-fpm 模型和 net/http 包的网络io 模型的区别
  ```
...
* mq 的使用场景
  ...
  ```
  1.削峰填谷。缓冲上下游瞬时突发流量，保护“脆弱”的下游系统不被压垮，避免引发全链路服务“雪崩”。（口碑项目保护算法提取关键词。
  2. 复用 。返现项目主要是外部公司暴露出去的一般用的都是api 项目，比如我们要获取订单，都是通过一个地方 job 拉订单，然后放到消息队列中，让各个业务方自行订阅消费）
...
* mq 的消息顺序问题，如何解决
  ...
  再根据我们 记录变动的时候存储的 消息时间，保证 先产生的消息一定不会在 后产生的消息之前执行，如果存在，直接丢弃，记录异常。
  ...
  1、发版导致的consumer变动
  2.partition 的增加
  3.partition 上消息堆积的时候
  ```

* 如何保证一个队列只有一个消费端

  ```
  kafka 的队列也就是topic  可以对应多个分区，多个分区对应多个消费者，但单个分区只能对应单个消费者，但是
  比如比如平滑重启的时候，会有消息重复的问题，分布式锁登场。
  
  感觉多个消费者无法保证offset 的顺序性， 所以一个partition 没法被多个offset 消费。
  ```
...
* producer 发送消息
  ...
  ack  1 , 必须有leader 副本应答才算完成
  
  ack 0, 不管谁应答都算完成
  
  ack -1 所有副本应答才算完成 （isr 集合内）
  ```
...
* kafka 为什么快
  ...
  ````
  1.利用了磁盘连续读写性能远远高于随机读写的特点，内部采用消息的批量处理，zero-copy （数据->read buff --> 应用层 buff-> socket buff）机制, 网卡直接访问内存，数据的存储和获取是本地磁盘顺序批量操作，具有 O (1) 的复杂度，消息处理的效率很高。
